...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (text: string, file?: { mimeType: string; data: string; }) => {
    if ((!text.trim() && !file) || !chat || !activeChat) return;
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-900 font-sans">
      ...
      <div className={`flex flex-1 overflow-hidden ${isMobile ? 'flex-col' : ''}`}>
        ...
        >
          <main
            className={`flex flex-col bg-zinc-950 min-w-0 ${isMobile ? 'w-full' : ''}`}
            style={!isMobile ? { width: `${panelsWidth.chat}%` } : {}}
          >
            {error ? (
              <div className="flex-1 flex items-center justify-center text-center text-zinc-500 p-4">
                <p>
                  <strong className="font-bold text-zinc-200">{t('errorLabel')}:</strong> {error}
                </p>
              </div>
            ) : (
              <ChatWindow
          ...
          {/* Hide resizer and preview on mobile */}
          {!isMobile && (
            <>
              <div
          ...
          {/* Show preview below chat on mobile */}
          {isMobile && (
